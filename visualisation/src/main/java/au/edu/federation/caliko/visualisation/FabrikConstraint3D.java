...
public class FabrikConstraint3D {
	...
	private static final float rotStep = 360.0f / (float) NUM_CONE_LINES;

	private final Circle3D mCircle; // Used to draw hinges
	private final Line3D mLine;   // Used to draw hinge axes, reference axes and ball-joint cones
	...
	private void draw(FabrikBone3D bone, Vec3f referenceDirection, float lineWidth, Mat4f mvpMatrix) {
		...
		switch (bone.getJoint().getJointType()) {
			case BALL: {
				...
				float pushDistance = (float) Math.cos(constraintAngleDegs * MathUtil.DEG_TO_RAD) * boneLength;
				float radius = (float) Math.sin(constraintAngleDegs * MathUtil.DEG_TO_RAD) * boneLength;
	 ...
	 * @param mvpMatrix The ModelViewProjection matrix to use.
	 */
	public void draw(FabrikChain3D chain, Mat4f mvpMatrix) {
		draw(chain, 1.0f, mvpMatrix);
	}
	 ...
	 * @param mvpMatrix The ModelViewProjection matrix to use.
	 */
	public void draw(FabrikStructure3D structure, Mat4f mvpMatrix) {
		draw(structure, 1.0f, mvpMatrix);
	}
