...
@State(Scope.Thread)
public class CalikoRotationMatrixBenchmarks {
...
class Experiments {
	static Vec3f X = new Vec3f(1f, 0f, 0f);
	static Vec3f Y = new Vec3f(0f, 1f, 0f);
	...
	static Mat3f createRotationMatrixBaseline(Vec3f referenceDirection) {
		...
		if (Math.abs(referenceDirection.y) > 0.9999f) {
			...
			rotMat.setZBasis(referenceDirection);
			rotMat.setXBasis(Vec3f.crossProduct(referenceDirection, Y).normalised());
			rotMat.setYBasis(Vec3f.crossProduct(rotMat.getXBasis(), referenceDirection).normalised());
		}
	...
	static Mat3f createRotationMatrixFast(Vec3f referenceDirection) {
		...
		if (Math.abs(referenceDirection.y) > 0.9999f) {
			...
			rotMat.setYBasis(Vec3f.crossProduct(X, referenceDirection).normalise()); // xBasis cross referenceDirection
		}
		else {
			rotMat.setZBasis(referenceDirection);
			Vec3f xBasis = Vec3f.crossProduct(referenceDirection, Y).normalise();
