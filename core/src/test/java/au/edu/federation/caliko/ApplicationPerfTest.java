...
public class ApplicationPerfTest
{
	...
	public void runTests() throws Exception
	{
		...
		writer = new PrintWriter(new File(tempDir, "caliko-performance-test.txt"), StandardCharsets.UTF_8);
		...
		writer.close(); // save results to file for import into Excel
	}

	private void performTest(int testNumber) throws Exception
	{
	...
	private void assertChain(FabrikChain3D chain, int testNumber) throws Exception {

		File file = new File(tempDir, "perftest-serialized-" + testNumber + ".bin");
		SerializationUtil.serializeChain( chain, new FileOutputStream( file ) );

		InputStream is = ApplicationPerfTest.class.getResourceAsStream("/serialization-presaved-chain-" + testNumber + ".bin");
		FabrikChain3D deserializedPresavedChain = SerializationUtil.unserializeChain(is, FabrikChain3D.class);

		assertEquals(chain, deserializedPresavedChain);
	}
